...
class LambdaLayer(nn.Module):
    """Wraps an arbitrary callable as an nn.Module for use inside nn.Sequential.

    Args:
        lambd: A callable that transforms the input tensor.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply the wrapped callable to the input.

        Args:
            x: Input tensor.

        Returns:
            Transformed tensor returned by lambd.
        """
        return self.lambd(x)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Output tensor of shape (N, channels * expansion, H_out, W_out).
        """
...
class ResNet(nn.Module):
    ...
    def _make_layer(
        ...
        """Build a residual layer composed of multiple blocks.
        ...
        Args:
            ...
            norm: Normalisation layer class passed to each block.
            stride: Stride applied to the first block only.

        Returns:
            nn.Sequential layer containing all blocks.
        """
