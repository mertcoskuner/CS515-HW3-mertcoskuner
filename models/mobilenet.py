...
class Block(nn.Module):
    ...
    Uses an expansion factor to increase channel width before depthwise
    convolution, then projects back to a lower dimension.  A residual
    connection is added only when stride == 1 and channel dimensions match.
    ...
    def __init__(self, in_planes: int, out_planes: int, expansion: int, stride: int) -> None:
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1, groups=planes, bias=False)  # groups=planes makes it depthwise
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through expand-depthwise-pointwise with optional shortcut.

        Args:
            x: Input tensor of shape (N, in_planes, H, W).

        Returns:
            Output tensor of shape (N, out_planes, H_out, W_out).
        """
...
class MobileNetV2(nn.Module):
    ...
    Strides and pooling kernel sizes are adjusted from the original ImageNet
    variant so that the spatial dimensions remain valid for 32x32 inputs.
    ...
    def _make_layers(self, in_planes: int) -> nn.Sequential:
        """Build all inverted residual stages from the cfg table.

        Args:
            in_planes: Number of input channels to the first block.

        Returns:
            nn.Sequential containing all inverted residual blocks.
        """
        layers = []
...
def test() -> None:
    """Smoke test: run a forward pass with random CIFAR-10-sized inputs."""
    net = MobileNetV2()
